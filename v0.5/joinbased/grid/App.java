		...
		button2.addActionListener(new ActionListener(){//运行的监听
			public void actionPerformed(ActionEvent e) {
				...
				if(num==0||avg==0){
					...
					Joinbase.candidate = new Candidate();
					Joinbase.point2Feature = new HashMap<Integer,Integer>();
					Joinbase.rules = new Rules();
					try {						
						Joinbase.setParameter(path, dis, pre, conf);
					...
					for(Rule ru : Joinbase.rules.list){
