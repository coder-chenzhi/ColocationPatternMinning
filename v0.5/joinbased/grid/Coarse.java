...
public class Coarse {
	...
	public void createCoarse(int num) {
		HashMap<CoFeatures,ArrayList<Integer[]>> candi2Instan =
										Joinbase.candidate.candi2Ins[num-1];
		HashMap<CoFeatures,HashSet<CoarseIns>> candi2Cos =
										Joinbase.coarse.candi2Coarse[num-2];
			...
			for(int i = 0; i < cosIns1.length; i++)
				for(int j = 0; j < cosIns2.length; j++){
					if(equalsExceptLast(cosIns1[i], cosIns2[j])){
						...
						tempIns.add(g1);
						tempIns.add(g2);
						if(Joinbase.coarse.candi2Coarse[1].get(tempcf).contains(tempIns)){
	...
	public void prevalentFiltration(int num){
		...
		for(CoFeatures cf : candi2Coarse.keySet()){
			...
			for(int i = 0; i < num; i++){
				...
				int total = Joinbase.candidate.candi2Ins[0].get(tempcf).size();
				int sum = 0;
			...
			if(pre < this.prevalent){
				delete.add(cf);
				Joinbase.candidate.candi2Ins[num-1].remove(cf);
			}
		}
		for(CoFeatures tempcf : delete)
			Joinbase.coarse.candi2Coarse[num-1].remove(tempcf);
	}
