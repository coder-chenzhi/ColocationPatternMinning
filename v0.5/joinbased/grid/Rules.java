...
public class Rules {
	...
	public void createRules(Candidate candi) {
		for (HashMap<CoFeatures,ArrayList<Integer[]>> tempMap : candi.candi2Ins) {
			if (tempMap.size() == 0) {
				break;
			}
			for (CoFeatures tempFea : tempMap.keySet()) {
				...
				for (int i = 1; i < (int)Math.pow(2, tempInte.length-1); i++) {
					...
					ArrayList<Integer[]> instances =
							candi.candi2Ins[tempFea.value.size()-1].get(tempFea);
					...
					double tempConfidence = (double)tempSet.size() /
							candi.candi2Ins[subFea.value.size()-1].get(subFea).size();
					...
					tempConfidence = (double)tempSet.size() /
							candi.candi2Ins[copyFea.value.size()-1].get(copyFea).size();
					if (tempConfidence > this.confidence)
